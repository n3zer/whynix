...
Item {
	...
	Item {
		...
		opacity: !root.toastActive && !Popups.dashboardOpen ? 1 : 0
		visible: opacity > 0
	...
	// ── Center toast — layout switch + clipboard copy ─────────────────────────
	// Replaces the Hyprland submap notifications: shows a small pill in the
	// middle of the notch for ~1.6s whenever the keyboard layout changes
	// (niri msg event-stream) or text is copied (wl-paste --watch).

	property string toastText:   ""
	property string toastIconChar: "ⓘ"
	property real   toastOpacity: 0
	readonly property bool toastActive: toastOpacity > 0
	...
	function showToast(icon, text) {
		root.toastIconChar = icon
		root.toastText     = text
		root.toastOpacity  = 1
		toastTimer.restart()
	}

	// ── niri 'grp' layouts — polled, since alt-shift toggles inside xkb ──────
	// niri doesn't emit event-stream events for `grp:alt_shift_toggle`
	// (layout changed by the xkb option, not niri). So we poll
	// `niri msg --json keyboard-layouts` and diff current_idx.
	property int _lastLayoutIdx: -1
	...
	function _pollLayout() {
		_layoutReq.running = false
		_layoutReq.running = true
	}
	...
	Process {
		...
		stdout: StdioCollector {
			onStreamFinished: function() {
				...
				if (!obj || typeof obj.current_idx !== "number") return
				var idx = obj.current_idx
				if (root._lastLayoutIdx === -1) {
					root._lastLayoutIdx = idx
					return
				}
				if (idx === root._lastLayoutIdx) return
				root._lastLayoutIdx = idx
				var name = (Array.isArray(obj.names) && idx >= 0 && idx < obj.names.length)
					? obj.names[idx] : ""
				var short = name
				if (/english/i.test(name))   short = "EN"
				else if (/russian|русск/i.test(name)) short = "RU"
				else {
					var mm = name.match(/\(([^)]*)\)/)
					if (mm) short = mm[1]
				}
	...
	// ── clipboard watch — fires on every new copy (wl-clipboard) ──────────────
	Process {
		id: clipWatcher
		command: ["bash", "-c", "wl-paste --type text --watch echo copy"]
		running: true
		onExited: function() { clipboardRestart.restart() }
		stdout: SplitParser {
			onRead: function(line) {
				if (line.indexOf("copy") !== -1) root.showToast("󰆏", "Copied")
			}
		}
	}
	Timer { id: clipboardRestart; interval: 1500; onTriggered: clipWatcher.running = true }

	// ── Center toast — layout switch + clipboard copy ─────────────────────────
	// Replaces the normal center text with "(icon + text)" for ~1.6s whenever
	// the keyboard layout changes (niri msg event-stream) or text is copied
	// (wl-paste --watch). Plain text only — no pill/background. The carousel
	// fades out underneath and comes back when the toast clears.
	Item {
		anchors.fill: parent
