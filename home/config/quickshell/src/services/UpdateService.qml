...
QtObject {
    ...
    // If the repo has no configured remote, updates don't apply — bail out
    // silently instead of surfacing a "could not reach remote" error popup.
    property var _fetchProc: Process {
        command: ["bash", "-c",
            "if ! git -C '" + root._dir + "' remote get-url origin >/dev/null 2>&1; then" +
            "  echo NO_REMOTE; exit 3; fi; " +
            "git -C '" + root._dir + "' fetch origin main --quiet"]
        running: false
        stdout: StdioCollector {
            id: fetchOut
        }
        onExited: function(code) {
            if (code === 3 || code === 128) {
                // Not a git repo or no 'origin' remote — nothing to auto-update.
                console.log("UpdateService: no git remote configured, update skipped.")
                root.checking      = false
                root.lastError     = ""
