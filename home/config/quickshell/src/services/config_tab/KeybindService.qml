...
QtObject {
    ...
    // ── niri binds cache ──────────────────────────────────────────────────
    // Parsed from ~/.config/niri/binds.kdl by list_niri_binds.py.
    // Refreshed each time a BindRow enters capture mode.
    property var _niriBinds: []

    property var _niriBindsProc: Process {
        command: ["python3",
                  Quickshell.shellDir + "/src/scripts/list_niri_binds.py"]
        running: false
        stdout: StdioCollector {
            onStreamFinished: {
                try   { root._niriBinds = JSON.parse(text.trim()) }
                catch (e) { root._niriBinds = [] }
            }
        }
    }

    function loadNiriBinds() {
        _niriBindsProc.running = false
        _niriBindsProc.running = true
    }

    // Stable lookups for the unified BindRow niri mode — keyed by the live
    // combo token ("Mod+Shift+T") so rows survive capture == the writable file.
    readonly property var _niriById:      (function() {
        var m = {}
    ...
    // Original combo snapshot (pre-capture) so we can restore after unbind.
    readonly property var _niriOrigById:  (function() {
        var m = {}
        var a = root._niriBinds
        for (var i = 0; i < a.length; i++) {
            var b = a[i]
            if (b.combo) m[b.combo] = { mods: b.mods, key: b.key }
        }
        return m
    })()

    // Apply a set of niri ops atomically: rewrites the writable live user file
    // + git repo copy, then re-lists so the UI refreshes.
    ...
    function applyNiriOps(ops) {
        ...
        _applyNiriProc.command = ["python3",
            ...
            "--live", root.homeDir + "/.local/state/niri/binds.user.kdl",
            "--repo", root.homeDir + "/dotfiles/home/config/niri/binds.user.kdl",
            json]
        _applyNiriProc.running = false
        _applyNiriProc.running = true
    }

    // ── niri writable bind targets ────────────────────────────────────────────
    // niri config is a read-only store symlink, so live niri binds live in a
    // writable user file (~/.local/state) that binds.kdl includes. The repo copy
    // is kept identical so edits persist through git (and repopulate the store
    // after a rebuild via home.activation seeding).
    ...
    // Snapshot of the ORIGINAL niri combo for each bug bind combo key — lets the
    // UI restore/rebind without knowing the original token (which may contain
    // flags like allow-when-locked). Captured on first load.
    ...
    // Apply a set of niri ops atomically via patch_niri_binds.py (writes live +
    // repo copy, niri auto-reloads through the include watcher). Ops schema:
