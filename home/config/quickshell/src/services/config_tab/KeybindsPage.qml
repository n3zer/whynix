...
Item {
    ...
    // ── niri ops (unified design — same BindRow, routed to binder ops) ─────────
    property var _niriPending: {}   // niriId/shell combo → { mods, key, origCombo }

    function _addNiriPending(ident, newMods, newKey, origCombo) {
        var copy = Object.assign({}, _niriPending)
        copy[ident] = { mods: newMods, key: newKey, origCombo: origCombo }
        _niriPending = copy
    }

    function _clearNiriPending(ident) {
        var copy = Object.assign({}, _niriPending)
        delete copy[ident]
        _niriPending = copy
    }

    function _applyNiriPending() {
        var ops = []
        var ks  = Object.keys(_niriPending)
        for (var i = 0; i < ks.length; i++) {
            var p = _niriPending[ks[i]]
            if (p.mods === "" && p.key === "")
                ops.push({ combo: p.origCombo, unbind: true })
            else
                ops.push({ combo: p.origCombo, mods: p.mods, key: p.key })
        }
        _niriPending = {}
        if (ops.length) KeybindService.applyNiriOps(ops)
    }
        ...
        Column {
            ...
            Item {
                ...
                Text {
                    ...
                    text:           "Active Keybinds (niri / binds.kdl)"
                    font.pixelSize: 9
            ...
            Repeater {
                id: niriList
                model: KeybindService._niriBinds
                delegate: BindRow {
                    required property var modelData
                    isNiri:      true
                    isReadOnly:  true
                    niriId:      modelData.combo
                    action:      ""
                    width:       _col.width
                }
            }
    ...
    component BindRow: Item {
        ...
        property bool   isReadOnly:  false    // display-only: hides capture/clear/reset (niri keeps its own editor)
        property string action:      ""
        property bool   isNiri:      false    // niri bind mode (source of truth: binds.user.kdl)
        property string niriId:      ""       // stable identity token (e.g. "Mod+T") that survives rebinds
        ...
        readonly property var _b: br.isNiri
            ? KeybindService._niriById[br.niriId]
            : KeybindService.keybinds[action]
        readonly property var _def: br.isNiri
            ? KeybindService._niriOrig[br.niriId]
            : KeybindService._defaults[action]
        ...
        readonly property bool _interactive: !br.isReadOnly && (root._capturing === "" || br.isCapturing)
        ...
        onIsCapturingChanged: {
            if (isCapturing) {
                ...
                br.capturedMods = ""
                br.capturedKey  = ""
                KeybindService.loadNiriBinds()   // refresh for conflict detection
        ...
        Item {
            ...
            Keys.onReleased: function(event) {
                ...
                if (k !== "") {
                    ...
                    // no pending-map conflict, no niri conflict
                    if (m !== ""
                        && KeybindService.wouldConflict(br.action, m, k) === ""
                        && KeybindService.wouldConflictNiri(m, k) === ""
        ...
        Item {
            ...
            Row {
				...
				// Clear bind
                Rectangle {
                    visible: !br.isReadOnly && br._pillText !== "Unbound"
                ...
                // Reset to default
                Rectangle {
                    visible: !br.isReadOnly && !br._isDefault
                    width: 22; height: 22; radius: 6
